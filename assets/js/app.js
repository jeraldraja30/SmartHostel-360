...
function showGlobalLoader() {
    ...
    if (loader && !loader.dataset.hiddenByRequest) {
...
const apiClient = {
    async request(path, options = {}) {
        if (!options.hideLoader) showGlobalLoader();
        ...
        try {
            ...
            if (!options.hideLoader) hideGlobalLoader();
        }
    },
    fetchHostelData() {
...
async function refreshHostelData(showFallbackAlert = false) {
    try {
        const token = localStorage.getItem('accessToken');
        if (!token) {
            console.warn('No access token available in localStorage. Skipping backend fetch and using demo data.');
            if (showFallbackAlert) {
                showAlert('Not logged in to backend. Running in demo mode with sample data.', 'warning');
            }
        ...
        console.error('Backend fetch failed. Using demo data. Error:', error);
        if (showFallbackAlert) {
            showAlert('Backend fetch failed. Running in demo mode with sample data.', 'warning');
        }
        backendAvailable = false;
        refreshAllDataViews();
    }
...
function mapOutpassFromApi(item) {
    ...
    return {
        ...
        // NEW: WhatsApp parent approval fields
        parentPhone: item.parent_phone || '',
        parentStatus: item.parent_status || 'pending',
        parentResponseTime: item.parent_response_time || '',
        parentApproved: !!item.parent_approved,
...
let currentUser = {
    ...
    username: "",
    email: "",
    hosteler_id: ""  // Populated after login for student accounts
};

// Login module selection
function selectLoginModule(module) {
    // Remove active from all modules
    ['student', 'warden', 'admin'].forEach(m => {
        const el = document.getElementById(`${m}-login-module`);
        if (el) el.classList.remove('active');
    });

    // Activate selected
    const target = document.getElementById(`${module}-login-module`);
    if (target) target.classList.add('active');

    // Set hidden user-type select
    const userTypeEl = document.getElementById('user-type');
    if (userTypeEl) userTypeEl.value = module;
}
...
async function submitApplication(event) {
    ...
    // Generate username (AdminXXX-XXXX format with random suffix to avoid collisions)
    ...
    const username = `Admin${newAdminNumber.toString().padStart(3, '0')}${randomSuffix}`;

    // Generate password (FirstName123 format)
    const password = `${firstName}123`;

    // Attempt to register in the backend
    try {
        showGlobalLoader();
        ...
        if (!res.ok) {
            ...
            let errorMsg = 'Failed to register with backend';
            if (errData.username) errorMsg = errData.username[0];
        ...
        if (resData.success === false) {
            showAlert(`Registration Failed: ${resData.error || 'Unknown error'}`, 'error');
            hideGlobalLoader();
            return;
        }
    } catch (err) {
        console.error('Backend fetch error during registration:', err);
        // Continue to save locally if backend is unavailable for demo purposes
        console.warn('Proceeding with local registration only.');
    } finally {
        hideGlobalLoader();
    }
...
// Prevent default link behavior for empty hashes to not break History API
...
function showPage(pageId, updateHistory = true) {
    // Update hash for History API support
    ...
    if (pageId === 'welcome' || pageId === 'application' || pageId === 'login') {
        ...
        pages.forEach(page => {
            ...
            // Show the requested public page, hide other public pages
            if (page.id === pageId) {
        ...
        // Hide app chrome when showing public pages
        const appChrome = document.getElementById('app-chrome');
        if (appChrome) appChrome.style.display = 'none';
    ...
    if (!currentUser.type) {
        // Not logged in — silently redirect to login page
        showPage('login');
        return;
    }
    ...
    // Find and activate the corresponding sidebar link by data-page attribute
...
// ─── Login Alert Helper ───────────────────────────────────────────────────────
// Shows alerts inside the login form container (visible while main is hidden).
function showLoginAlert(message, type) {
    const container = document.getElementById('login-alert-container');
    if (!container) {
        // Fallback to browser alert if container missing
        console.warn('[Login] Alert container not found, using window.alert:', message);
        return;
    }
    const colorMap = { error: '#f8d7da', success: '#d4edda', warning: '#fff3cd' };
    const borderMap = { error: '#f5c6cb', success: '#c3e6cb', warning: '#ffeaa7' };
    const textMap   = { error: '#721c24', success: '#155724', warning: '#856404' };
    container.innerHTML = `
        <div style="
            padding: 12px 16px;
            border-radius: 6px;
            background: ${colorMap[type] || colorMap.error};
            border: 1px solid ${borderMap[type] || borderMap.error};
            color: ${textMap[type] || textMap.error};
            font-size: 14px;
            display: flex;
            align-items: center;
            gap: 8px;
...
async function login() {
    ...
    if (!usernameInput || !passwordInput) {
        console.error('[Login] Could not find username/password fields in DOM');
        showLoginAlert('Page error: login fields not found. Please refresh.', 'error');
        return;
    }

    const username = usernameInput.value.trim();
    const password = passwordInput.value;
    const userType = userTypeInput ? userTypeInput.value : 'student';
    ...
    try {
        ...
        if (res.ok) {
            ...
            // Backend returns success:false with HTTP 200 for bad credentials
            if (data.success === false) {
                console.warn('[Login] Backend returned success=false:', data.error);
            ...
            backendLoginOk = true;
            console.log('[Login] JWT tokens stored successfully');

            // Set current user from backend response
            const u = data.user || {};
            currentUser.type        = u.role || userType;
            currentUser.name        = [u.first_name, u.last_name].filter(Boolean).join(' ') || u.username || username;
            currentUser.username    = u.username || username;
            currentUser.email       = u.email || '';
            currentUser.hosteler_id = u.hosteler_id || '';  // Links student to Hosteler profile

            // ── Debug: confirm what was received ──────────────────────────────
            console.log('[Login] Raw user payload from backend:', u);
            console.log(`[Login] hosteler_id received: "${u.hosteler_id}" → stored as: "${currentUser.hosteler_id}"`);
            console.log('[Login] currentUser set to:', currentUser);

        } else if (res.status === 400 || res.status === 401) {
            const errData = await res.json().catch(() => ({}));
            console.warn('[Login] Auth rejected by backend:', errData);
            let errorMsg = 'Invalid credentials. Please try again.';
            if (errData.non_field_errors) errorMsg = errData.non_field_errors[0];
            else if (errData.detail)      errorMsg = errData.detail;
            ...
            console.warn(`[Login] Unexpected status ${res.status} from backend.`);
            showLoginAlert(`Unexpected server response (${res.status}). Please try again.`, 'warning');
            return;
        }
    } catch (err) {
        console.error('[Login] Network/fetch error:', err);
        showLoginAlert(
            'Cannot reach the server. Please make sure the Django backend is running at ' +
    ...
    if (!backendLoginOk) {
        console.warn('[Login] Login flow ended without success — stopping.');
        return;
    }
    ...
    // Persist user info
    localStorage.setItem('currentUserType', currentUser.type);
    localStorage.setItem('currentUserName', currentUser.name);
    localStorage.setItem('currentUserHostelerId', currentUser.hosteler_id || '');

    // Hide public pages
    document.getElementById('login').style.display = 'none';
    document.getElementById('welcome').style.display = 'none';

    // Show main application chrome (sidebar layout)
    const appChrome = document.getElementById('app-chrome');
    if (appChrome) appChrome.style.display = 'flex';

    // Update user display name in sidebar footer
    const userDisplay = document.getElementById('user-display');
    if (userDisplay) userDisplay.textContent = currentUser.name;

    // Update role label in sidebar footer chip
    const roleLabel = document.getElementById('sidebar-role-label');
    if (roleLabel) roleLabel.textContent = currentUser.type.charAt(0).toUpperCase() + currentUser.type.slice(1);

    // Apply role-based UI restrictions and navigate to dashboard
    applyUserRestrictions();

    // Load live backend data
    refreshHostelData(true);

    // Show success alert (now main is visible, showAlert will work)
    showAlert(`Welcome back, ${currentUser.name}! Login successful.`, 'success');
}

// Apply user restrictions based on user type
function applyUserRestrictions(navigate = true) {
    ...
    if (role === 'warden') {
        ...
        show('boys-allocation-menu');
        show('girls-allocation-menu');
        show('boys-allocation-card');
        show('girls-allocation-card');
        show('warden-notifications-menu');
        show('warden-inbox-menu');
        show('verify-menu');
        ...
        hide('features-menu');
        hide('contact-menu');
        hide('room-management-card');
        hide('student-notifications-menu');
        hide('complaint-card');
        hide('feedback-card');

        if (navigate) showPage('warden-dashboard');

    // ── Admin view ──────────────────────────────────────────────────────────
    } else if (role === 'admin') {
        // Admin sees everything the warden sees
        show('admin-menu');
        hide('warden-menu');
        show('room-management-menu');
        show('boys-allocation-menu');
        ...
        // Admin also gets access to student management
        ...
        hide('contact-menu');
        hide('student-notifications-menu');
        // Admin can see all dashboard cards
        show('room-management-card');
        show('complaint-card');
        show('feedback-card');

        // ── Admin-specific dashboard UI ─────────────────────────────────────
        const heading = document.getElementById('warden-dash-heading');
        if (heading) heading.innerHTML = '<i class="fas fa-user-cog"></i> Admin Dashboard';
    ...
    // ── Student view ─────────────────────────────────────────────────────────
        ...
        hide('warden-menu');
        hide('admin-menu');
        hide('room-management-menu');
        hide('boys-allocation-menu');
        hide('girls-allocation-menu');
        hide('boys-allocation-card');
        hide('girls-allocation-card');
        hide('warden-notifications-menu');
        hide('warden-inbox-menu');
        hide('verify-menu');
        ...
        show('contact-menu');
        show('room-management-card');
        show('student-notifications-menu');
        show('complaint-card');
        show('feedback-card');

        // ── Reset admin-specific dashboard UI ───────────────────────────────
        const heading = document.getElementById('warden-dash-heading');
        if (heading) heading.innerHTML = '<i class="fas fa-user-shield"></i> Warden Dashboard';
        const banner = document.getElementById('admin-role-banner');
        if (banner) banner.style.display = 'none';
        const actionsPanel = document.getElementById('admin-actions-panel');
        if (actionsPanel) actionsPanel.style.display = 'none';

        if (navigate) showPage('home');
...
// Admin helper: fetch all students via backend API and refresh
async function adminFetchAllStudents() {
    const token = localStorage.getItem('accessToken');
    if (!token) { showAlert('Not authenticated. Please login again.', 'error'); return; }
    try {
        showGlobalLoader();
        ...
        showAlert(`✅ Student data refreshed — ${count} student(s) loaded from database.`, 'success');
        await refreshHostelData(false);
    } catch (err) {
        showAlert(`❌ Failed to fetch student data: ${err.message}`, 'error');
    } finally {
        hideGlobalLoader();
    }
}
...
function logout() {
    if (confirm('Are you sure you want to logout?')) {
        ...
        // Hide main application (sidebar layout)
        const appChrome = document.getElementById('app-chrome');
        if (appChrome) appChrome.style.display = 'none';
        ...
        // Clear stored user type and JWT tokens from localStorage
        localStorage.removeItem('currentUserType');
        localStorage.removeItem('currentUserName');
        localStorage.removeItem('currentUserHostelerId');
        localStorage.removeItem('accessToken');
        localStorage.removeItem('refreshToken');

        // Reset login modules — default to student
        ['student', 'warden', 'admin'].forEach(m => {
            const el = document.getElementById(`${m}-login-module`);
            if (el) el.classList.remove('active');
        });
        const studentMod = document.getElementById('student-login-module');
        if (studentMod) studentMod.classList.add('active');
        const userTypeEl = document.getElementById('user-type');
        ...
        .map(outpass => `
            <li class="activity-item">
                ...
                <div style="display: flex; gap: 5px;">
                    <button class="btn btn-success btn-small" onclick="approveOutpass('${outpass.id}')" title="Accept" ${outpass.parentStatus !== 'approved' ? 'disabled style="opacity: 0.5; cursor: not-allowed;"' : ''}>
        ...
        <div class="outpass-card">
            <div class="outpass-status">
                ...
                <div style="display: flex; gap: 10px; align-items: center;">
                    <span class="status-badge ${outpass.status === 'approved' ? 'status-available' : outpass.status === 'pending' ? 'status-pending' : 'status-occupied'}" title="Warden Status">
                        Warden: ${outpass.status}
                    </span>
                    <span class="status-badge ${outpass.parentStatus === 'approved' ? 'status-available' : outpass.parentStatus === 'pending' ? 'status-pending' : 'status-occupied'}" title="Parent WhatsApp Status" style="background-color: ${outpass.parentStatus === 'pending' ? '#e0f2fe' : (outpass.parentStatus === 'approved' ? '#dcfce7' : '#fee2e2')}; color: ${outpass.parentStatus === 'pending' ? '#0369a1' : (outpass.parentStatus === 'approved' ? '#166534' : '#991b1b')}; margin-left: 5px;">
                        <i class="fab fa-whatsapp"></i> Parent: ${outpass.parentStatus || 'pending'}
                    </span>
                </div>
            </div>
            ...
            ${currentUser.type === 'warden' && outpass.status === 'pending' ? `
                <div class="outpass-actions">
                    <button class="btn btn-success btn-small" onclick="approveOutpass('${outpass.id}')" ${outpass.parentStatus !== 'approved' ? 'disabled style="opacity: 0.5; cursor: not-allowed;" title="Waiting for Parent Approval"' : ''}>
...
// Submit outpass request (Original)
function submitOutpass() {
    ...
    showAlert('Outpass Applied Successfully by ' + studentName, 'success');
...
// NEW: Enhanced submit outpass request handling updated HTML form IDs
window.submitOutpassEnhanced = async function() {
    const studentName = document.getElementById('outpass-student-name').value.trim();
    const parentPhone = document.getElementById('outpass-parent-phone')?.value.trim();
    const outDate = document.getElementById('outpass-date-time').value;
    const returnDate = document.getElementById('outpass-return-date-time').value;
    const reason = document.getElementById('outpass-reason').value;
    const details = document.getElementById('outpass-details').value;
    ...
    if (!parentPhone) {
        showAlert('Please enter parent WhatsApp number', 'error');
        return;
    }
    ...
    // Get hosteler_id if current user is a student (needed by backend to link Hosteler record)
    let studentId = '';
    if (window.currentUser && window.currentUser.type === 'student') {
        studentId = window.currentUser.hosteler_id || '';
    }
    ...
    // If backend is active, optionally send to django API
    if (typeof apiClient !== 'undefined' && backendAvailable) {

        // ── Safety guard: student must have a hosteler_id to create an outpass ──
        if (currentUser.type === 'student' && !studentId) {
            console.error('[Outpass] ❌ Cannot submit: student is missing hosteler_id.', currentUser);
            showAlert(
                '⚠️ Your account is not linked to a hosteler profile. ' +
                'Please contact the warden to link your Hosteler ID (e.g. H2024001) to your account.',
                'error'
            );
            return; // Stop — do not save locally or call API
        }

        console.log('[Outpass] Submitting to backend with payload:', {
            hosteler_id: studentId,
            parent_phone: parentPhone,
            out_date: outDate.split('T')[0],
        ...
        try {
            const apiRes = await apiClient.createOutpass({
                hosteler_id: studentId,
                parent_phone: parentPhone,
                out_date: outDate.split('T')[0],
                return_date: returnDate.split('T')[0],
            ...
            console.log('[Outpass] ✅ Backend response:', apiRes);
            newOutpass.backendId = apiRes.id;
            newOutpass.whatsappMessageId = apiRes.whatsapp_message_id;
        } catch (err) {
            console.error('[Outpass] ❌ Backend error creating outpass:', err);
            showAlert('Saved locally, but backend sync failed: ' + (err.message || 'Unknown error'), 'warning');
        }
    }
    ...
    // Customized success message with user name
    showAlert('Outpass Applied Successfully by ' + studentName, 'success');
...
function approveOutpass(outpassId) {
    ...
    if (outpass) {
        if (outpass.parentStatus !== 'approved') {
            showAlert('Cannot approve outpass: Parent has not approved yet!', 'error');
            return;
        }
        ...
        // If backend is active, optionally send to django API
        if (typeof apiClient !== 'undefined' && backendAvailable && outpass.backendId) {
            apiClient.setOutpassStatus(outpass.backendId, { status: 'approved' }).catch(err => {
                console.error("Failed to approve outpass on backend", err);
            });
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Safely set default dates – these elements may not exist on initial load
    ...
    const outpassDate = document.getElementById('outpass-date');
    if (outpassDate) outpassDate.value = today;

    // Set return date to tomorrow by default
    const tomorrow = new Date();
    tomorrow.setDate(tomorrow.getDate() + 1);
    const outpassReturnDate = document.getElementById('outpass-return-date');
    if (outpassReturnDate) outpassReturnDate.value = tomorrow.toISOString().split('T')[0];
    ...
    // We now enforce that a JWT token MUST be present to restore the session.
    // This prevents old mock 'demo' sessions from persisting and triggering errors.
    if (storedType && token) {
        currentUser.type        = storedType;
        currentUser.name        = localStorage.getItem('currentUserName') || 'User';
        currentUser.hosteler_id = localStorage.getItem('currentUserHostelerId') || '';
        
        // Hide login and welcome pages
        const welcomePage = document.getElementById('welcome');
        if (welcomePage) welcomePage.style.display = 'none';
        const loginPage = document.getElementById('login');
        if (loginPage) loginPage.style.display = 'none';

        // Show main application containers
        ...
        // Apply restrictions and fetch data
        if (typeof applyUserRestrictions === 'function') applyUserRestrictions(false);
        if (typeof refreshHostelData === 'function') {
            const token = localStorage.getItem('accessToken');
            refreshHostelData(!!token);
        }
    }
...
window.addEventListener('popstate', function() {
    ...
    if (!pageId) {
        pageId = currentUser.type ? (currentUser.type === 'warden' ? 'warden-dashboard' : 'home') : 'welcome';
    }
    ...
    feedbackList.innerHTML = hostelData.feedback.map(feedback => `
        <div class="feedback-item">
            ...
            <div class="feedback-message">
                <strong>${(feedback.type || 'other').charAt(0).toUpperCase() + (feedback.type || 'other').slice(1)}:</strong> ${feedback.message || ''}
            </div>
            ${feedback.reply ? `
